...
public final class JOB003 {
    // Run CALCFEE from CALCFEE.cbl; do not simplify its conditions.
    static void program_CALCFEE(MfRuntime.Context ctx, Map<String,Object> dd, MfRuntime.Ref[] arguments) throws Exception {
        var s = new MfRuntime.Fields(ctx.layout("CALCFEE"));
        var streams = new HashMap<String,MfRuntime.RecordFile>();
        if (arguments.length != 3) throw new IllegalArgumentException("CALL argument count differs from source linkage.");
    ...
    static void program_FEEPOST(MfRuntime.Context ctx, Map<String,Object> dd, MfRuntime.Ref[] arguments) throws Exception {
        ...
        streams.put("INFILE", ctx.open(MfRuntime.obj(dd.get("SORTED")), "INPUT", s.length("IN-RECORD")));
        streams.put("OUTFILE", ctx.open(MfRuntime.obj(dd.get("FEES")), "OUTPUT", s.length("OUT-RECORD")));
