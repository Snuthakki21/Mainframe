...
preserves text width, and writes to a fresh SQLite test database. No AI sees or
...
from .common import Blocked, inside
...
class Context:
    ...
    def __init__(self, payload: dict):
        ...
        self.conn = sqlite3.connect(str(self.work_root/'local.sqlite'))
    ...
    def insert(self, table: str, columns: list[str], values: list[Any]) -> None:
        """Insert bound values; exact decimals are stored as text, never SQLite REAL."""
        if len(columns) != len(values): raise ValueError('SQL columns and values have different counts.')
        if table not in self.schema: raise ValueError(f'No authoritative schema was supplied for {table}.')
        converted = []
        for col, value in zip(columns,values):
            definition = self.schema[table][col]
            if definition['kind'] == 'integer':
                n = Decimal(value)
                if n != n.to_integral_value(): raise ValueError(f'{table}.{col} received a fractional integer.')
                number = int(n)
                if not -(2**63) <= number < 2**63: raise ValueError('Integer does not fit SQLite storage.')
                converted.append(number)
            elif definition['kind'] == 'decimal':
                if isinstance(value,float): raise TypeError('Binary floats are not accepted.')
                n = Decimal(value)
                quantum = Decimal(1).scaleb(-definition['scale'])
                if n != n.quantize(quantum): raise ValueError(f'{table}.{col} needs explicit source rounding/truncation before insert.')
                if abs(n) >= Decimal(10)**(definition['precision']-definition['scale']):
                    raise ValueError(f'{table}.{col} exceeds its declared precision.')
                converted.append(format(n, f'.{definition["scale"]}f'))
            else:
                text = str(value)
                if len(text) > definition['length']: raise ValueError(f'{table}.{col} exceeds its declared width.')
                converted.append(text)
        quoted = ','.join('"'+c+'"' for c in columns)
        sql = f'INSERT INTO "{table}" ({quoted}) VALUES ({",".join("?" for _ in columns)})'
        self.conn.execute(sql,converted)
    def commit(self) -> None:
        """Commit only when the translated source explicitly requests it."""
        self.conn.commit()
    def rollback(self) -> None:
        self.conn.rollback()
    def end_step(self) -> None:
        """Close run-unit files at a JCL step boundary; do not carry buffered writes forward."""
        self.finish()
        for stream in self.opened:
            if not stream.handle.closed: stream.close()
        self.opened.clear()
    def finish(self) -> None:
        """Reject an unspecified end-of-job transaction rather than silently committing it."""
        if self.conn.in_transaction:
            self.conn.rollback()
            raise ValueError('The job ended with an open transaction. Its end-of-job commit behavior needs an explicit source-backed plan.')
    def close(self) -> None:
        for stream in self.opened:
            if not stream.handle.closed: stream.close()
